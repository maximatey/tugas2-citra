function blpf = blpf(image)
    ...
    % Step 4: Bangkitkan fungsi penapis H untuk Butterworth Lowpass Filter
    ...
    H = 1 ./ (1 + (D ./ D0).^(2*n)); % Fungsi Butterworth
    H(D0 == 0) = 0; % Menangani kasus D = 0
    H = fftshift(H); % Pindahkan pusat ke frekuensi
    
    figure; imshow(H); title('BLPF Butterworth Mask');
    figure, mesh(H);
    ...
    blpf = G2(1:M, 1:N); % Resize the image untuk menghapus padding
    
    % Tampilkan citra output
    figure; imshow(blpf); title('Output image after inverse 2D DFT');
end